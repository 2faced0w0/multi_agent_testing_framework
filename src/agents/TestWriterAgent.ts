...
export class TestWriterAgent extends BaseAgent {
  ...
  // Generate test using Mistral AI chat completions
      let testCode = await this.generateTestCode(url, testType, description);

      // Mandate validation of generated test code by CodeWatcher before storing
      let validatedCode = '';
      try {
        await this.sendMessage('logger_1', 'LOG', {
          level: 'info',
          message: 'Sending test code to CodeWatcher API for validation',
          data: { url, testType, description, testCode }
        });
        const res = await fetch('http://localhost:3000/api/v1/codewatcher/validate', {
          method: 'POST',
        ...
        await this.sendMessage('logger_1', 'LOG', {
          level: 'info',
          message: 'Received response from CodeWatcher API',
          data: { fixedCode: data.fixedCode }
        });
        if (data.fixedCode && typeof data.fixedCode === 'string' && data.fixedCode.trim().length > 0) {
          validatedCode = data.fixedCode;
        }
      } catch (err) {
        console.error('Error validating test code via CodeWatcher API:', err);
        await this.sendMessage('logger_1', 'LOG', {
          level: 'error',
          message: 'Error validating test code via CodeWatcher API',
          data: { error: err instanceof Error ? err.message : String(err) }
        });
      }
      await new Promise(resolve => setTimeout(resolve, 12000));
      if (!validatedCode || validatedCode.trim().length === 0) {
        console.error('[TestWriterAgent] Test code validation by CodeWatcher failed or returned empty. Aborting storage.');
        throw new Error('Test code validation by CodeWatcher failed or returned empty.');
      }
      testCode = validatedCode;
      ...
      try {
        await this.storeData(`testcase:${testCase.creation_id}`, testCase);
        console.log('[TestWriterAgent] Stored test case in Redis:', testCase.creation_id);
        await this.sendMessage('logger_1', 'LOG', {
          level: 'info',
          message: '[TestWriterAgent] Stored test case in Redis',
          data: { creation_id: testCase.creation_id }
        });
      } catch (redisError) {
      ...
      try {
        console.log('[TestWriterAgent] Attempting to store test case in SQLite:', testCase.creation_id, testCase);
        await this.sendMessage('logger_1', 'LOG', {
          level: 'info',
          message: '[TestWriterAgent] Attempting to store test case in SQLite',
          data: { creation_id: testCase.creation_id, testCase }
        });
        this.dbManager.createTestCase(testCase);
        console.log('[TestWriterAgent] Successfully stored test case in SQLite:', testCase.creation_id);
        await this.sendMessage('logger_1', 'LOG', {
          level: 'info',
      ...
      await this.sendMessage('logger_1', 'LOG', {
        ...
        data: { target: message.source, creation_id: testCase.creation_id }
      });
